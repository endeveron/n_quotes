...
function FormTextarea({
  ...
  return (
    <textarea
      data-slot="textarea"
      className={cn(
        'w-full p-2 flex font-semibold border-2 border-input-border rounded-sm bg-input placeholder:font-normal placeholder:text-sm placeholder:leading-6 placeholder:text-muted field-sizing-content outline-none focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:ring-error/80 aria-invalid:border-error/80 trans-c',
        className
      )}
      {...props}
    />
...
function FormMessage({ className, ...props }: React.ComponentProps<'p'>) {
  ...
  return (
    <p
      ...
      id={formMessageId}
      className={cn(
        'mt-0 text-sm font-bold text-error leading-snug tracking-wide',
        className
      )}
      {...props}
    >
